...
void main()
{
    ...
    if (texture(material.specular, TexCoord).rgb == vec3(0.0f))
    {
        ...
        new_text.t += gTime;
        //emissionVal = texture(emission, new_text).rgb * 3.0f;
    }
    ...
    float d = length(FragPos.xyz - light.position);

    float kc = 1.0f / (1 + light.linear * d + light.quadratic * (d * d));
    vec3 result = (ambient + diffuse
            + specular + emissionVal) * kc;
